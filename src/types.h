...
#define DIFFUS                                                                 \
  1.3E-8 // Diffusivity of chlorine
         // @ 20 deg C (sq ft/sec)
#define VISCOS                                                                 \
  1.1E-5                 // Kinematic viscosity of water
                         // @ 20 deg C (sq ft/sec)
...
typedef enum { NODE, LINK, TIMEPAT, CURVE, CONTROL, RULE } ObjectType;

typedef enum { JUNCTION, RESERVOIR, TANK } NodeType;
...
typedef struct // Control Rule Structure
{
  ...
  // Turbo Optimizations
  int is_time_only;       // 1 if rule depends ONLY on time
  int64_t next_eval_time; // Next time implementation check is needed
} Srule;
...
typedef struct {
  ...
  void (*reportCallback)(void *, void *,
                         const char *); // user-supplied reporting callback
...
typedef struct {
  ...
  SflowBalance FlowBalance; // Flow balance components
...
typedef struct {
  ...
  int step_count;
  int iter_count;
  int rules_eval_count;
  int rules_fire_count;
  int rules_skip_count;
  int simple_controls_eval_count;
  int simple_controls_fire_count;
  int simple_controls_skip_count;
} Profile;
...
typedef struct Project {
  ...
  Profile prof; // Turbo profiling stats
